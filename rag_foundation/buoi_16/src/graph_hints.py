...
def get_graph_hints(results: list[dict], limit_per_result: int = 10) -> tuple[list[dict], str]:
    try:
        ...
        MATCH (v:VanBan {id:item.document_id, lab_session:'buoi_16'})
        OPTIONAL MATCH (v)-[vr {lab_session:'buoi_16'}]->(target)
        WITH item, v, collect(DISTINCT {from:v.id, relation:type(vr), to:target.id})[0..$limit] AS doc_relations
        OPTIONAL MATCH (d:DieuKhoan {id:item.chunk_id, lab_session:'buoi_16'})-[dr {lab_session:'buoi_16'}]->(next)
